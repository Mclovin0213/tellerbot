...
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.answer('All added products in this category.')
...
async def process_body_back(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.answer(f"Change name from <b>{data['title']}</b>?", reply_markup=back_markup())
...
async def process_image_url(message: Message, state: FSMContext):
    ...
    if message.text == back_message:
        ...
        async with state.proxy() as data:
            ...
            await message.answer(f"Change description from <b>{data['body']}</b>?", reply_markup=back_markup())
...
async def process_confirm_back(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.answer(f"Change price from <b>{data['price']}</b>?", reply_markup=back_markup())
...
async def show_products(m, products, category_idx):
    ...
    await m.answer('Do you want to add or remove something?', reply_markup=markup)
